...
class EHtmlDateSelect extends CWidget {
    ...
    public function init() {
        ...
        if (!isset($this->prefix))
            $this->prefix = "Date_";
        if (!isset($this->start_year) or empty($this->start_year))
            $this->start_year = strftime("%Y");
        if (!isset($this->end_year) or empty($this->end_year))
            $this->end_year = $this->start_year;
        if (!isset($this->display_days))
            $this->display_days = true;
        if (!isset($this->display_months))
            $this->display_months = true;
        if (!isset($this->display_years))
            $this->display_years = true;
        if (!isset($this->month_format) or empty($this->month_format))
            $this->month_format = "%B";
        /* Write months as numbers by default  GL */
        if (!isset($this->month_value_format) or empty($this->month_value_format))
            $this->month_value_format = "%m";
        if (!isset($this->day_format) or empty($this->day_format))
            $this->day_format = "%02d";
        /* Write day values using this format MB */
        if (!isset($this->day_value_format) or empty($this->day_value_format))
            $this->day_value_format = "%d";
        if (!isset($this->year_as_text))
            $this->year_as_text = false;
        /* Display years in reverse order? Ie. 2000,1999,.... */
        if (!isset($this->reverse_years))
            $this->reverse_years = false;
        /* Should the select boxes be part of an array when returned from PHP?
          e.g. setting it to "birthday", would create "birthday[Day]",
          "birthday[Month]" & "birthday[Year]". Can be combined with prefix */
        if (!isset($this->field_array))
            $this->field_array = null;
        /* <select size>'s of the different <select> tags.
          If not set, uses default dropdown. */
        if (!isset($this->day_size))
            $this->day_size = null;
        if (!isset($this->month_size))
            $this->month_size = null;
        if (!isset($this->year_size))
            $this->year_size = null;
        /* Unparsed attributes common to *ALL* the <select>/<input> tags.
          An example might be in the template: all_extra ='class ="foo"'. */
        if (!isset($this->all_extra))
            $this->all_extra = null;
        /* Separate attributes for the tags. */
        if (!isset($this->day_extra))
            $this->day_extra = null;
        if (!isset($this->month_extra))
            $this->month_extra = null;
        if (!isset($this->year_extra))
            $this->year_extra = null;
        /* Order in which to display the fields.
          "D" -> day, "M" -> month, "Y" -> year. */
        if (!isset($this->field_order) or empty($this->field_order))
            $this->field_order = 'MDY';
        /* String printed between the different fields. */
        if (!isset($this->field_separator))
        ...
        if (!isset($this->month_empty))
            $this->month_empty = null;
        if (!isset($this->year_empty))
            $this->year_empty = null;
    }
    ...
    public function run() {
        ...
        for ($i = 0; $i <= 2; $i++) {
            ...
            switch ($c) {
                case 'D': $html_result .= $day_result;
                    break;
                case 'M': $html_result .= $month_result;
                    break;
                case 'Y': $html_result .= $year_result;
                    break;
            }
    ...
    protected function html_options($params) {
        ...
        foreach ($params as $_key => $_val) {
            switch ($_key) {
                case 'name': $$_key = (string) $_val;
                    break;
                case 'options': $$_key = (array) $_val;
                    break;

                case 'values':
                case 'output': $$_key = array_values((array) $_val);
                    break;

                case 'selected': $$_key = array_map('strval', array_values((array) $_val));
                    break;
