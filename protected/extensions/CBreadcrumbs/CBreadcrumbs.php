...
class CBreadcrumbs extends CWidget {
    ...
    public function run() {
        ...
        if ($this->homeLink === null)
            $links[] = '<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb">' . CHtml::link('<span itemprop="title">' . Yii::t('zii', 'Home') . '</span>', Yii::app()->homeUrl, array('itemprop' => 'url')) . '<span class="divider">/</span></li>';
        else if ($this->homeLink !== false)
            $links[] = $this->homeLink;
        foreach ($this->links as $label => $url) {
            if (is_string($label) || is_array($url)) {
                $a_title = $this->encodeLabel ? CHtml::encode($label) : $label;
                $links[] = '<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb">' . CHtml::link('<span itemprop="title">' . $a_title . '</span>', $url, array('itemprop' => 'url')) . '<span class="divider">/</span></li>';
            }
            else
                $links[] = '<li class="active" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">' . ($this->encodeLabel ? CHtml::encode($url) : $url) . '</li>';
        }
