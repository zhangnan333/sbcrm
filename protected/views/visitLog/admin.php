...
$this->breadcrumbs=array(
	'Visit Logs'=>array('site/index'),
	'管理',
);
Yii::app()->clientScript->registerScript('search', "
$('form.search-form').submit(function(){
	$.fn.yiiGridView.update('visit-log-grid', {
		data: $(this).serialize()
	});
	return false;
});
$('#delete-button').click(function(){
    var selects = $.fn.yiiGridView.getSelection('visit-log-grid');
    if(selects==''){
        alert('请选中记录进行操作！');
    }else{
        ...
        <div class="inbox-sf">
            <form class="search-form">
				...
				$this->Widget('ext.searchField.searchFieldWidget',array(
                    'initText'=>'',
                    'titleText'=>'输入XXXX查询',
                    'inputName'=>'VisitLog[xxx]',
                    'inputId'=>'VisitLog_xxx_search',
                ));?>
	...
	'dataProvider' => $model->search(),
		...
		'visit_date',
		 array(
                    ...
                    'value'=>'Yii::app()->controller->getColumnOptions($data->id,"VisitLog")',
                ),
	),
)); ?>         
    </div><!-- END ".box-content" --> 
