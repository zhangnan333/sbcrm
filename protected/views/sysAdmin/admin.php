...
$this->breadcrumbs = array(
    '系统用户' => array('sysAdmin/admin'),
    '管理',
);
Yii::app()->clientScript->registerScript('search', "
$('form.search-form').submit(function(){
	$.fn.yiiGridView.update('sys-admin-grid', {
		data: $(this).serialize()
	});
	return false;
});
$('#delete-button').click(function(){
    var selects = $.fn.yiiGridView.getSelection('sys-admin-grid');
    if(selects==''){
        alert('请选中记录进行操作！');
    }else{
    ...
    <div class="box-content" id="contacts-1a">
        <div class="inbox-sf">
            <form class="search-form">
                ...
                $this->Widget('ext.searchField.searchFieldWidget', array(
                    'initText' => '',
                    'titleText' => '输入用户名查询',
                    'inputName' => SysAdmin . '[user_name]',
                    'inputId' => SysAdmin . '_user_name_search',
                ));
        ...
        $this->widget('zii.widgets.grid.CGridView', array(
            ...
            'columns' => array(
                ...
                array(
                    'name' => 'rid',
                    'value' => 'SysRole::model()->getNameByPk($data->rid)',
                ),
                /*
                  'last_ip',
                 */
                array(
                    ...
                    'value' => 'Yii::app()->controller->getColumnOptions($data->id,"SysAdmin")',
                ),
            ),
        ));
        ?>         
